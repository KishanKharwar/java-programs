...
public class RealTimeQuestions {
    ...
    private static void separateByAge25() {
        Map<Boolean,List<Employee>> partition = employeeList.stream().collect(Collectors.partitioningBy(e->e.getAge()>25));
    ...
    private static void nameOfEmpsInEachDept() {
        Map<String,List<Employee>> names = employeeList.stream().collect(Collectors.groupingBy(Employee::getDepartment));
    ...
    private static void mostWorkingExp() {
        Employee e = employeeList.stream().sorted(Comparator.comparing(Employee::getYearOfJoining)).findFirst().get();
        System.out.println(e);
    }

    private static void getYoungestMaleInProductDept() {
       Optional<Employee> emp =  employeeList.stream().filter(e->e.getDepartment()== "Product Development" && e.getGender() == "Male").min(Comparator.comparingInt(Employee::getAge));
